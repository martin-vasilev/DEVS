...
Time in ms between sending the command to play the sound and the end of the saccade that triggered the sound: Mean: `r round(mean(sound_check$delFix, na.rm=T),1)`. SD: `r round(sd(sound_check$delFix, na.rm=T),1)`. Range: `r range(sound_check$delFix, na.rm=T)`. 
...
```{r, echo= FALSE}
hist(sound_check$delFix, xlab= "Sound onset delay (in ms) relative to fixation onset of the target word", main= "Sound implementation timing", family= "serif", col="lightgreen", freq=FALSE, breaks=30, xlim= c(-20, 20)) # , xlim= c(-20, 20)
...
```
*Note*: Extreme values due to long saccades are removed from this graph (`r round((length(which(sound_check$delFix< -20))/ nrow(sound_check))*100, 1)` %).
...
if(office){
  ...
  load("C:/Users/Martin Vasilev/Documents/DEVS/data/raw_fix.Rda")
}
...
Greater regression probability is usually associated with difficulty in sentence processing. For example, if word processing was affected somehow when participants heard a noise, they may be more likely to make a regression back to it and re-read it.

### Number of fixations on target region:

The sound manipulation may affect not only the word on which a sound is played, but also the next word (where no sound is played). I use the term 'target region' to refer to these two words together. For example, since we play sounds on the 3rd, 5th, 7th, 9th, and 11th word, the first target region would be word 3 + word 4, the second target region would be word 5 + word 6 and so on. 

```{r, echo=FALSE}
if(office){
  load("C:/Users/mvasilev/Documents/DEVS/data/FixRG.Rda")
} else{
  load("C:/Users/Martin Vasilev/Documents/DEVS/data/FixRG.Rda")
}

FixRG$sound<- as.factor(FixRG$sound)

DesRG<- melt(FixRG, id=c('sub', 'item', 'cond', 'sound'), 
              measure=c("nfix1", "nfix2", "nfixAll"), na.rm=TRUE)
mRG<- cast(DesRG, sound ~ variable
            , function(x) c(M=signif(mean(x),3)
                            , SD= sd(x) ))
mRG[,c(3,5,7)]<- round(mRG[,c(3,5,7)],2)

colnames(mRG)<- c('Sound', 'First-pass (mean)', 'First-pass (SD)', 'Second-pass (mean)', 'Second-pass (SD)',
                  'All (mean)', 'All (SD')
#mRG$Sound<- factor(mRG$Sound, levels=c("DEV", "STD", "SLC"))
levels(mRG$Sound)<- c('Standard', 'Deviant', 'Silence')
```
