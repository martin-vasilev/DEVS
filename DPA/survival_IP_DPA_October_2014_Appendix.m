function [divergence_point] = survival_IP_DPA_October_2014_Appendix(straps) %enter the number of bootstraps.
...
for s = 1:subs; %for each subject, associate the x variable with the A condition data and the y variable with the B condition data (note: A is the fast condition, and B is the slow condition).
      ...
      if sum(srvl_straps_was_there_divergence)>0; %check if at least one of the straps produced a divergence point             
         ...
         median_srvl_duration_at_divergence_bin = -999999; %The 999999 "dummy value" indicates that there were no significant divergence points for this subject.
         sorted_srvl_duration_at_divergence_bin = -999999;  % The 999999 "dummy value" indicates that there were no significant divergence points for this subject.
         srvl_duration_at_divergence_bin_ci_upper = -999999; % The 999999 "dummy value" indicates that there were no significant divergence points for this subject.
         srvl_duration_at_divergence_bin_ci_lower = -999999; % The 999999 "dummy value" indicates that there were no significant divergence points for this subject.
         median_srvl_divergence_bin = 999999; % The 999999 "dummy value" indicates that there were no significant divergence points for this subject.      
      end
      
      
      
        %store the DPA results from each participant in a .txt file
        diffsmatrix_output = [diffsmatrix_output; s, median_srvl_divergence_bin, median_srvl_duration_at_divergence_bin, srvl_duration_at_divergence_bin_ci_upper, srvl_duration_at_divergence_bin_ci_lower, count_of_straps_with_divergence_points]; %display the current output to the user.
