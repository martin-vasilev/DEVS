function [divergence_strap_median] = survival_CI_DPA_October_2014_Appendix(straps) %enter the number of bootstraps.
...
%input file path.
dat_dir = 'C:/Users/Martin Vasilev/Documents/DEVS/DPA/'; %location of the data input spreadsheet
...
divergence_strap_vector =[];
    ...
    for bs = 1:straps
       ...
       run_count = 0;
        ...
        for b = 1:nbins
            if srvl_straps(3,b,bs)>= DV_point_empirical_criterion;  % in this case, we are checking to see where the lower bound is greater than zero
                splitpoint = b;
       ...
       if  divergence_point_strap > 0; %if there is a divergence point...
           ...
           % compute area measure
            area_between_strap = sum(srvl_straps(2,divergence_point_strap:end,bs))-sum(srvl_straps(1,divergence_point_strap:end,bs));
            %area_under_high_strap = sum(srvl_straps(1,divergence_point_strap:end,bs));
            area_under_low_strap = sum(srvl_straps(2,divergence_point_strap:end,bs)); 

            area_measure_strap = area_between_strap/area_under_low_strap;

            DV_strap_matrix = [DV_strap_matrix; bs, divergence_point_strap, percentage_below_strap, area_measure_strap]; %save all straps in a matrix
       else
            %calculate percentage below divergence
            percentage_below_strap = -4;

           % compute area measure
            area_between_strap = -4;
            %area_under_high_strap = -4;
            area_under_low_strap = -4;

            area_measure_strap = -4;
